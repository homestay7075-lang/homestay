...
export default function EditStudentModal({
  ...
  useEffect(() => {
    if (student && isOpen) {
      ...
      setIdProofDocumentUrl(student.idProofDocumentUrl || '');
      setNewPassword('');
      setErrorMsg('');
      setSuccessMsg('');
    }
  ...
  const handleIdFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setErrorMsg(`ID document error: ${err.message}`);
    }
  };
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSave} className="overflow-y-auto p-6 space-y-5 flex-1">
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
            ...
            {/* ID Proof Card Upload */}
            <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200">
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                  <FileText className="w-3.5 h-3.5 text-indigo-600" />
                  <span>ID Card / Document Photo</span>
              ...
              {idProofDocumentUrl ? (
                <div className="flex items-center gap-3 bg-white p-2.5 rounded-xl border border-slate-200">
                  <a
                    href={idProofDocumentUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="relative w-16 h-12 rounded-lg overflow-hidden border border-slate-200 shrink-0 group bg-slate-100 block"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-slate-800 truncate">ID Document Image</p>
                    <p className="text-[11px] text-slate-500 truncate">Attached to resident profile</p>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    onClick={() => idFileInputRef.current?.click()}
                    className="flex-1 min-w-[130px] inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-slate-300 hover:border-indigo-400 hover:bg-indigo-50/40 text-slate-700 text-xs font-semibold rounded-xl transition shadow-xs"
                  >
                    <Upload className="w-3.5 h-3.5 text-indigo-600" />
