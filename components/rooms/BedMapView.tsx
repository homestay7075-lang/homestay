      ...
      {activeBedModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
            ...
            {/* Modal Footer */}
            <div className="px-6 py-3.5 border-t border-slate-100 bg-slate-50 flex items-center justify-between gap-2 flex-wrap shrink-0">
              <button
                type="button"
                onClick={() => {
                ...
                className={`py-1.5 px-3 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition ${
                  activeBedModal.status === 'Occupied' || activeBedModal.status === 'Vacating'
                    ? 'text-slate-300 border-slate-200 cursor-not-allowed'
                    : 'text-rose-600 border-rose-200 bg-rose-50/50 hover:bg-rose-100 cursor-pointer'
                }`}
                title={
                  activeBedModal.status === 'Occupied'
