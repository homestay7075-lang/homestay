...
function OwnerProfileContent() {
  ...
  const handleToggleBiometrics = async () => {
    ...
    if (isBiometricEnabled) {
      disableBiometrics();
      setBioFeedback({ text: 'Phone Lock & Fingerprint security disabled for management dashboard.', isError: false });
    } else {
      setTogglingBio(true);
      const res = await enableBiometrics();
      setTogglingBio(false);
      if (res.success) {
        setBioFeedback({ text: 'Fingerprint & Phone Lock enabled for management dashboard!', isError: false });
      } else {
        setBioFeedback({ text: res.error || 'Failed to activate phone lock.', isError: true });
      }
    }
  };
        ...
        {activeTab === 'SECURITY' && (
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xs p-6 sm:p-8 space-y-6">
            ...
            <form onSubmit={handleSaveSecurity} className="space-y-5">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3">
                <span className="text-xs font-bold text-slate-800 uppercase tracking-wider block">
                  Additional Account Protection & Biometrics
                </span>

                <div className="space-y-3 text-xs">
                  {/* Phone Lock & Fingerprint Card */}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-slate-900 block text-xs">Phone Lock & Fingerprint</span>
                          {isBiometricEnabled && (
                  ...
                  {bioFeedback && (
                    ...
                    >
                      ...
                      <span className="text-[11px] font-medium">{bioFeedback.text}</span>
                    </div>
                  )}

                  {isBiometricEnabled && (
                    <div className="flex items-center justify-between p-2.5 bg-purple-50/50 border border-purple-100 rounded-xl">
                      <span className="text-[11px] text-purple-900 font-medium">
                        Biometric lock active. Click to test screen lock immediately:
                      </span>
                      <button
                        type="button"
                        onClick={() => lockApp()}
                        className="px-2.5 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs font-bold transition flex items-center gap-1 shadow-xs"
                      >
